...
use crate::helpers::helpers::to_dense;
...
impl SparseMat {
    ...
    pub(crate) fn sort_indices_data_slices(&mut self) {
        let mut buf = Vec::new();
        let len = self.indices.len();
        assert_eq!(len, self.data.len());
        let indices = &mut self.indices[..len];
        let data = &mut self.data[..len];
        buf.clear();
        ...
        for (&(i, x), (ind, v)) in buf.iter().zip(indices.iter_mut().zip(data.iter_mut())) {
            *ind = i;
            *v = x;
        }
    }

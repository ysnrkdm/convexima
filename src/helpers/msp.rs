...
use crate::problem::{ComparisonOp, LinearExpr, OptimizationDirection, Problem, Variable};
...
";
    ...
    fn test_parse_mps_file() {
        ...
        assert_eq!(sol.objective_value(), 54.0);
        assert_eq!(*sol.var_value(file.variables["XONE"]), 4.0);
        assert_eq!(*sol.var_value(file.variables["YTWO"]), -1.0);
        assert_eq!(*sol.var_value(file.variables["ZTHREE"]), 6.0);
    }
}
